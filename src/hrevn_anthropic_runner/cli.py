#!/usr/bin/env python3
"""Installable CLI bridge for Anthropic-facing HREVN workflows."""
...
def require_key(value: str | None) -> str:
    if value:
        return value
    raise SystemExit("HREVN_API_KEY is required. Run `hrevn-anthropic health-check` first if you only want connectivity.")
...
def build_parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(description="Call the HREVN managed API from Claude Code and Anthropic-facing workflows.")
    root.add_argument(
        "--base-url",
